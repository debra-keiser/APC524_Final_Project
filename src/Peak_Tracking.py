...
for i in range(1, total_datasets):
    ...
    while p_tracked < len(tracked_matrix[i - 1, :]) and p_ori < len(file[keys[i]][:]):
        diff = calc_diff(i, p_tracked, file[keys[i]][p_ori])
        if abs(diff) <= threshold_distance:
            ...
            if diff2 < diff:
                ...
                tracked_matrix[i, p_tracked] = file[keys[i]][p_ori]
                p_ori += 1
                p_tracked += 1
    ...
    if tracked_matrix[i, p_tracked - 1] != file[keys[i]][p_ori - 1]:
...
np.savetxt(
    "tracked_peaks_matrix.txt", tracked_matrix, delimiter="|"
)  # problem here: scientific notation is used when saved as text.
