...
from reportlab.platypus import SimpleDocTemplate, PageBreak, Paragraph, Image, Spacer, Table
...
def create_report(file_path):
    ...
    # Title
    title = Paragraph("<b>Automated Pair Distribution Function Analysis for Assessing Reaction Progress</b>", styles['Title'])
    story.append(title)

    centered_style = ParagraphStyle(name='Centered', alignment=TA_CENTER)
    author_names = Paragraph("<i>Authors: Sophia Bergen, Debra Keiser, and Meddelin Setiawan</i>", centered_style)
    ...
    # SECTION 1: PLOTTING
    story.append(Paragraph('<a name="page2"/>Section 1: PDF Curve Plotting', styles['Heading1']))
    ...
    story.append(Paragraph('<a name="page3"/>Section 2: Visualizing Total Number of Peaks Over Time ', styles['Heading1']))
    story.append(Paragraph('This section visualizes phase changes by counting the total number of peaks in PDF files across temperatures', styles['Normal']))
    ...
    story.append(Paragraph('<a name="page4"/>Section 3: Quantifying Peak Positions', styles['Heading1']))
    story.append(Paragraph('Contents of Section 3', styles['Normal']))
    story.append(PageBreak())

    #SECTION 4: PEAK INTEGRATION
    story.append(Paragraph('<a name="page4"/>Section 4: Peak Integration', styles['Heading1']))
    story.append(Paragraph('Contents of Section 3', styles['Normal']))
