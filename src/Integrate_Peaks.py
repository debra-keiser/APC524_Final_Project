...
def peak_integration(dwell_peaks_dict):
    ...
    for dwell_temperature in dwell_peaks_dict:
        if int(dwell_temperature) >= 100:
            ...
            trapezoidal_integrals = [integrate_peak_areas(pdf_dwell_data_g_r, peak) for peak in dwell_peaks_dict[dwell_temperature]]
    ...
    reference_dwell_peak_integrals_dict, dwell_peak_integral_differences_dict = peak_integral_differences(dwell_peak_integrals_dict, dwell_peaks_dict)
...
def scale_peak_integrals(peak_integrals_dict):
    ...
    reference_dwell_temperature = (list(peak_integrals_dict.keys()))[0]
    ...
    for dwell_temperature in peak_integrals_dict:
        if dwell_temperature != reference_dwell_temperature:
            Si_O_scale_factor = reference_Si_O_integral / (peak_integrals_dict[dwell_temperature])[0]
            peak_integrals_dict[dwell_temperature] = np.multiply(peak_integrals_dict[dwell_temperature], Si_O_scale_factor)
...
def peak_integral_differences(peak_integrals_dict, peaks_dict):
    ...
    for peak_position in peaks_dict[reference_dwell_temperature]:
        if peak_position < 500:
            reference_peak_integrals_dict[peak_position] = (peak_integrals_dict[reference_dwell_temperature])[np.where(peaks_dict[reference_dwell_temperature] == peak_position)[0][0]]
            ...
            for ref_position in reference_peak_positions:
                for shifted_position in peaks_dict[dwell_temperature]:
                    if shifted_position in range(ref_position - 15, ref_position + 16):
                        integral_differences_list.append(abs((peak_integrals_dict[dwell_temperature])[np.where(peaks_dict[dwell_temperature] == shifted_position)] - reference_peak_integrals_dict[ref_position])[0])
                    else:
                        None
            peak_integral_differences_dict[dwell_temperature] = np.array(integral_differences_list)
